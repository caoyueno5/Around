...
func checkUser(username, password string) bool {
	es_client, err := elasetic.NewClient(Elastic.SetURL(ES_URL), elastic.SetSniff(false))
	if err != nil {
...
func signupHandler(w http.ResponseWriter, r *http.Request) {
	...
	if u.Username != "" && u.Password != nil && usernamePattern(u.Username) {
		if addUser(u) {
		...
		http.Error(w, "Empty password or username or invalid username.", http.Status.InternalServerError)
	}
